...
| **Native listeners on `.react-flow__pane`** | The pane **contains** `.react-flow__viewport`, so every node, handle and edge-label event bubbles through it. xyflow's own `onPane*` props check the target for you; a hand-rolled `addEventListener` does not, and a "canvas gesture" will fire on nodes (US-133 tap-hold did). Guard with `if (e.target !== pane) return` — empty canvas hits the pane directly |
| **Security headers need a browser pass** | `curl` only proves a header is *present*. US-078 shipped COEP `require-corp` that passed curl and broke hydration; US-141's CSP must be checked the same way: `bun run build && bun run start`, load the editor in a real browser, and confirm the console has zero CSP (or COEP/CORP) violations across the surfaces the story names |
