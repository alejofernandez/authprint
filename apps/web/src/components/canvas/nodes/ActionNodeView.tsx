...
export function ActionNodeView({ data, selected }: ActionNodeProps) {
  ...
  return (
    <CanvasNodeRoot
      ...
      className={`group relative rounded-md bg-sky-50 dark:bg-sky-950/40 border border-sky-300 dark:border-sky-800 border-t-4 border-t-sky-500 dark:border-t-sky-400 ${canvasNodeRing(data.diagnostics, data.unwired)} ${canvasNodeOpacity()}`}
    >
